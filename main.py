...
# 다운 파일 파이참 폴더로 옮기고 pip install dlib-19.17.0-cp37-cp37m-win_amd64.whl     # pip install face_recognition 설치
import argparse
import cv2
import pickle
from PIL import Image, ImageFont, ImageDraw, ImageEnhance       # PIL : Python Imaging Library
import os
import sklearn.neighbors._base
from sklearn.neighbors import KNeighborsClassifier


# argparse : Python Script 실행 시, 하나의 Script 동작을 여러가지 상황에 따라 다르게 동작하도록 (여러 개 데이터(정해지지 않은 데이터)를 다루고 싶을 때)
parser = argparse.ArgumentParser()    # 파서(parser) : 그 문장의 구조를 알아내는 구문 분석(parsing)
parser.add_argument('--with_draw', help='do draw?', default='True')
# 인수명을 'arg'와 같이 일반적으로 지정하면 arg가 된다.
args = parser.parse_args()
#  dnn 모듈에서의 readNetFromCaffe 함수를 사용해, caffeframework의 네트워크 모델을 읽어들인다. -> (정의한 caffemodel과 prototxt를 인자로 받아와서) 하나의 Net 객체를 반환하게 된다.
net = cv2.dnn.readNetFromCaffe('./models/deploy.prototxt.txt', './models/res10_300x300_ssd_iter_140000.caffemodel')     #DNN은 입력층(input layer)과 출력층(output layer) 사이에 여러 개의 은닉층(hidden layer)들로 이뤄진 인공신경망(Artificial Neural Network, ANN)
knn_clf = pickle.load(open('./models/fr_knn.pkl', 'rb'))

def adjust_gamma(image, gamma=1.0):  # 감마 조정
    # build a lookup table mapping the pixel values [0, 255] to - 픽셀 값 [0, 255]를 매핑하는 조회 테이블을 작성
    invGamma = 1.0 / gamma        # their adjusted gamma values - 조정된 감마 값
    table = np.array([((i / 255.0) ** invGamma) * 255
                      for i in np.arange(0, 256)]).astype("uint8")

    # apply gamma correction using the lookup table - 조회 테이블 사용하여 감마 보정 적용
...
cap = cv2.VideoCapture('./data/with_Maeng_1.mp4')  # 사용할 동영상 넣으면 결과로 이 영상에서 모자이크 처리와 라벨링 보여줌
# 비디오 캡쳐. 동영상을 cap에 저장. 원 코드 vc를 cap으로 이름만 바꿔줌
...
out = cv2.VideoWriter('output_label.avi', fcc, fps, (width, height))  # '저장할 파일명' - 라벨링
out_2 = cv2.VideoWriter('output_blur.avi', fcc, fps, (width, height))   # '저장할 파일명' - 블러
...
for idx in range(length):
    ...
    for name, bbox, conf in list_reconized_face:
        ...
        if name == 'unknown':
            ...
            small = cv2.resize(face, None, fx=.07, fy=.07, interpolation=cv2.INTER_NEAREST)       # fx=.05, fy=.05 -> fx=.07, fy=.07 : 숫자 키워야 블러 픽셀 감소
...
#
#     else :
#         print("Fail to read frame!")          # 프레임이 끝나면 이 구문이 출력
#         break
